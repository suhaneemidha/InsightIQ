...
from ui.components import sidebar_stat, section_header, footer
from datetime import datetime

st.set_page_config(
    page_title="InsightIQ Knowledge Graph",
    page_icon="🕸️",
    layout="wide"
)
...
render_page_banner(
    ...
    subtitle="Interactive visualization of customers, orders, products and reviews",
    height=180,
)
st.markdown(
...
for _, row in items_df.iterrows():
    oid = row["order_id"]
    pid = row["product_id"]
    raw_cat = row["product_category_name"]
    if raw_cat is None or not isinstance(raw_cat, str):
        raw_cat = "unknown"
...
conn.close()
footer("InsightIQ • Knowledge Graph")